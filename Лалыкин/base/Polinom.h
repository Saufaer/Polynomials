...
{	
	...
	int len=st.length();//вернул длину строки
	int i=0;
	while (i!=len)//пока не дойдём до конца строки
	{
		if((s[i]=='-')&&(i==0))//если это первый моном с отрицательным коэффициентом
		{
			num+='-';
			i++;
		}

		if ((s[i] >= '0' && s[i] <= '9')&&(i==0))//если коэффициент в первом
		{
			num+=s[i];
			i++;
		}

		if ((s[i] >= '0' && s[i] <= '9')&&(i!=0)&&(s[i-1]!='x')&&(s[i-1]!='y')&&(s[i-1]!='z'))//если не в первом
		{
			num+=s[i];
			i++;
		}
